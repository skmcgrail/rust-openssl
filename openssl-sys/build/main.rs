...
fn check_ssl_kind() {
    ...
    let aws_lc_fips = cfg!(feature = "aws-lc-fips");
    let aws_lc = cfg!(feature = "aws-lc");

    let mutually_exclusives_count = vec![boringssl, aws_lc_fips, aws_lc]
        .iter()
        .filter(|x| **x)
        .count();

    if mutually_exclusives_count > 1 {
        eprint!("unstable_boringssl, aws-lc-fips, aws-lc are mutually exclusive crate features");
        std::process::exit(1);
    }
